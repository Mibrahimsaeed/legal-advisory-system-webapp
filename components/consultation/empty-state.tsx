    ...
    <div className="mx-auto flex w-full max-w-2xl flex-col gap-8 px-4 py-10 sm:px-6 sm:py-16">
      <div className="flex flex-col gap-3">
        ...
        <p className="leading-7 text-muted-foreground">
          Ask a direct legal question or describe a legal scenario. Legal Intelligence will identify relevant
          Pakistani statutes, case law, and supporting evidence.
        </p>
      </div>
      <ul className="flex flex-col gap-3">
        {EXAMPLE_PROMPTS.map(({ kind, text }) => (
          <li key={kind}>
            <button
              type="button"
              onClick={() => onPick(text)}
